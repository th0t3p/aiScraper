...
Protocol summary:
  1. GET {sse_path}               → long-lived SSE stream
...
Supports both official PortSwigger Burp MCP Server (sse_path="/sse")
and BurpMCP-Ultra (sse_path="/", requires Authorization header).
"""
...
class McpSseClient:
    ...
    ----------
    base_url:
        Base URL of the MCP server (e.g. "http://127.0.0.1:9876").
    timeout:
        Timeout in seconds for the SSE handshake and RPC calls.
    headers:
        Optional HTTP headers to include on every request (SSE and RPC POST).
        Useful for BurpMCP-Ultra which requires ``Authorization: Bearer <token>``.
    sse_path:
        Path for the SSE endpoint.  Default ``"/sse"`` (PortSwigger official).
        Set to ``"/"`` for BurpMCP-Ultra.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._sse_path = sse_path if sse_path.startswith("/") else "/" + sse_path
